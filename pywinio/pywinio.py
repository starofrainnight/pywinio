...
IOCTL_WINIO_MAPPHYSTOLIN = winioctlcon.CTL_CODE(
    FILE_DEVICE_WINIO,
    WINIO_IOCTL_INDEX,
...
IOCTL_WINIO_UNMAPPHYSADDR = winioctlcon.CTL_CODE(
    FILE_DEVICE_WINIO,
    WINIO_IOCTL_INDEX + 1,
    winioctlcon.METHOD_BUFFERED,
    winioctlcon.FILE_ANY_ACCESS,
...
IOCTL_WINIO_ENABLEDIRECTIO = winioctlcon.CTL_CODE(
    FILE_DEVICE_WINIO,
    WINIO_IOCTL_INDEX + 2,
    winioctlcon.METHOD_BUFFERED,
    winioctlcon.FILE_ANY_ACCESS,
...
IOCTL_WINIO_DISABLEDIRECTIO = winioctlcon.CTL_CODE(
    FILE_DEVICE_WINIO,
    WINIO_IOCTL_INDEX + 3,
    winioctlcon.METHOD_BUFFERED,
    winioctlcon.FILE_ANY_ACCESS,
...
IOCTL_WINIO_READPORT = winioctlcon.CTL_CODE(
    FILE_DEVICE_WINIO,
    WINIO_IOCTL_INDEX + 4,
    winioctlcon.METHOD_BUFFERED,
    winioctlcon.FILE_ANY_ACCESS,
...
IOCTL_WINIO_WRITEPORT = winioctlcon.CTL_CODE(
    FILE_DEVICE_WINIO,
    WINIO_IOCTL_INDEX + 5,
    winioctlcon.METHOD_BUFFERED,
    winioctlcon.FILE_ANY_ACCESS,
...
class WinIO(object):
        ...
        if self.__is_64bit_os():
            ...
            if bSize == 1:
                return ctypes.c_ubyte(
                    ctypes.cdll.msvcrt._inp(ctypes.c_ushort(wPortAddr))
                ).value
            elif bSize == 2:
                return ctypes.c_ushort(
                    ctypes.cdll.msvcrt._inpw(ctypes.c_ushort(wPortAddr))
                ).value
            elif bSize == 4:
                return ctypes.c_ulong(
                    ctypes.cdll.msvcrt._inpd(ctypes.c_ushort(wPortAddr))
                ).value
    ...
    def __set_port_value(self, wPortAddr, dwPortVal, bSize):
        ...
        if self.__is_64bit_os():
            ...
            win32file.DeviceIoControl(
                self.hDriver, IOCTL_WINIO_WRITEPORT, memoryview(PortStruct), 4
            )
        else:
            if bSize == 1:
                ctypes.cdll.msvcrt._outp(
                    ctypes.c_ushort(wPortAddr), ctypes.c_int(dwPortVal)
                )
            elif bSize == 2:
                ctypes.cdll.msvcrt._outpw(
                    ctypes.c_ushort(wPortAddr), ctypes.c_ushort(dwPortVal)
                )
            elif bSize == 4:
                ctypes.cdll.msvcrt._outpd(
                    ctypes.c_ushort(wPortAddr), ctypes.c_ulong(dwPortVal)
                )
    ...
    def get_phys_long(self, pbPhysAddr):
        ...
        dwLinAddrRaw = bytes((ctypes.c_char * 4).from_address(pdwLinAddr))
    ...
    def set_phys_long(self, pbPhysAddr, dwPhysVal):
        ...
        data = ctypes.cast(
            ctypes.c_void_p(pdwLinAddr), ctypes.POINTER(ctypes.c_ulong)
        )
